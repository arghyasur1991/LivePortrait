...
class MotionExtractorWrapper(nn.Module):
    """
    Wrapper for motion_extractor export from PyTorch
    """
    ...
    def forward(self, img):
        """
        Forward pass for motion extractor
        Returns individual tensors instead of dictionary to match ONNX format
        """
        output_dict = self.motion_extractor(img)
        # Return individual tensors in the expected order: pitch, yaw, roll, t, exp, scale, kp
        return (
            output_dict['pitch'],
            output_dict['yaw'],
            output_dict['roll'],
...
def export_motion_extractor_from_pytorch(output_path):
    """Export motion_extractor directly from PyTorch modules"""
    print(f"🔧 Exporting motion_extractor from PyTorch modules...")
    ...
    try:
        ...
        # Create motion extractor wrapper
        pytorch_wrapper = MotionExtractorWrapper(
            motion_extractor=wrapper.motion_extractor
        )
        ...
        # Fixed input shapes for maximum optimization
        with torch.no_grad():
            img = torch.randn(1, 3, 256, 256)  # Standard input size for motion extractor

            # Test
            test_output = pytorch_wrapper(img)
            print(f"✅ Test output shapes: pitch={test_output[0].shape}, yaw={test_output[1].shape}, roll={test_output[2].shape}")
            print(f"                        t={test_output[3].shape}, exp={test_output[4].shape}, scale={test_output[5].shape}, kp={test_output[6].shape}")
        ...
        try:
            ...
            if from_pytorch and model_name == "warping_spade":
                ...
                if export_warping_spade_from_pytorch(fp32_path):
                    ...
                    return 0
            elif from_pytorch and model_name == "motion_extractor":
                print(f"🔧 Exporting {model_name} from PyTorch modules...")
                if export_motion_extractor_from_pytorch(fp32_path):
                    print(f"✅ PyTorch export successful")
                else:
                    print(f"❌ PyTorch export failed")
                    return 0
